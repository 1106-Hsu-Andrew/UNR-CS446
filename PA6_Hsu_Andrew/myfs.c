...
void my_creatdir(myfs_t* myfs, int cur_dir_inode_number, const char* new_dirname) {
  // Find the first location that can be used for the new inode.
  block_t* imapData = (block_t*)(malloc(BLKSIZE));
  int newDirINode = -1;
  memcpy(imapData, (void*)&myfs->imap, BLKSIZE);
  for(uint i = 0; i < BLKSIZE * 8; i++){
    if((imapData->data[i / 8] & (0x1 << (i % 8))) == 0){
      imapData->data[i / 8] |= 0x1 << (i % 8);
      newDirINode = i;
      break;     
    }
  }
  memcpy((void*)&myfs->imap, imapData, BLKSIZE);

  // Find the first location that can be used as a data block.
  block_t* bmapData = (block_t*)(malloc(BLKSIZE));
  int newDirDBlock = -1;
  memcpy(bmapData, (void*)&myfs->bmap, BLKSIZE);
  for(uint i = 0; i < BLKSIZE * 8; i++){
    if((bmapData->data[i / 8] & (0x1 << (i % 8))) == 0){
      bmapData->data[i / 8] |= 0x1 << (i % 8);
      newDirDBlock = i;
      break;     
    }
  }
  memcpy((void*)&myfs->bmap, bmapData, BLKSIZE);

  // Access inode table to load parent directory and new directory inode.
  inode_t* inodeTable = (inode_t*)(malloc(BLKSIZE));
  ...
  inodeTable[newDirINode].size = 2 * sizeof(dirent_t);
  inodeTable[newDirINode].blocks = 1;
  for(uint i = 1; i < 15; i++){
    inodeTable[newDirINode].data[i] = NULL;
  }
  inodeTable[newDirINode].data[0] = &(myfs->groupdescriptor.groupdescriptor_info.block_data[newDirDBlock]);
  memcpy((void*)myfs->groupdescriptor.groupdescriptor_info.inode_table, inodeTable, BLKSIZE);

  //Access parent directory's data.
  dirent_t* parDir = (dirent_t*)(malloc(BLKSIZE));
  memcpy(parDir, inodeTable[cur_dir_inode_number].data[0], BLKSIZE);

  dirent_t* parNewDir = &parDir[(inodeTable[cur_dir_inode_number].size / sizeof(dirent_t)) - 1];
  parNewDir->name_len = strlen(new_dirname);
  parNewDir->inode = newDirINode;
  parNewDir->file_type = 2;
  strcpy(parNewDir->name, new_dirname);
  memcpy((void*)inodeTable[cur_dir_inode_number].data[0], parDir, BLKSIZE);

  dirent_t* newDir = (dirent_t*)(malloc(BLKSIZE));
  memcpy(newDir, inodeTable[newDirINode].data[0], BLKSIZE);

  dirent_t* newDirSelf = &newDir[0];
  newDirSelf->name_len = 1;
  newDirSelf->inode = newDirINode;
  newDirSelf->file_type = 2;
  strcpy(newDirSelf->name, ".");

  dirent_t* newDirPar =  &newDir[1];
  newDirPar->name_len = 2;
  newDirPar->inode = cur_dir_inode_number;
  newDirPar->file_type = 2;
  strcpy(newDirPar->name, "..");
  memcpy(inodeTable[newDirINode].data[0], newDir, BLKSIZE);
